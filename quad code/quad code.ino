...
float Gyro_angle_x, Gyro_angle_y,Gyro_angle_z;         		    //Here we store the angle value obtained with Gyro data(x-pitch,y-roll,z-yaw)
 ...
 * I make the mean of 200 values, that should be enough*/
  if(gyro_error==0)
  {
    for(int i=0; i<200; i++)
    {
      ...
      Wire.endTransmission(false);
     Wire.requestFrom(MPU_addr,(uint8_t)6,true);         //We ask for just 4 registers 
...
void loop() {
  ...
  //////////////////////////////////////Gyro read/////////////////////////////////////
    ...
    Wire.write(0x43);                        			//First adress of the Gyro data
    Wire.endTransmission(false);
    Wire.requestFrom(MPU_addr,(uint8_t)6,true);   //We ask for just 6 registers
  ...
  Wire.requestFrom(MPU_addr,(uint8_t)6,true);    			//We ask for next 6 registers starting withj the 3B 
...
after that we substract the error value found before*/  
 ...
 /*---X axis angle---*/
 Total_angle_x = 0.98 *(Total_angle_x + Gyro_angle_x) + 0.02*Acc_angle_x; //pitch
 /*---Y axis angle---*/
 Total_angle_y = 0.98 *(Total_angle_y + Gyro_angle_y) + 0.02*Acc_angle_y; //roll
